...
if [ $? != 0 ]; then #como teste, é válido esse tipo de PING, porém caso acontecesse uma falha nos servers de DNS do Google, o script retornaria sem Internet erroneamente.
    ...
    if [ -z "$CIDADE" ]; then
        ...
        #echo $RESPONSE #Deixei aqui para testes rápidos, caso necessário - Johnatas
        ...
        if [ $NOT_FOUND_ERROR == '"cod":"404"' ] 2> /dev/null; then  #resolvendo mensagem de falha de operador unário - Johnatas
            ...
            PAIS=$(grep -o -E '"country":[^,]+' $TMP_FILE | grep -o -E "[A-Z]{2}") #Colocar o país por conta de que algumas cidades tem nomes iguais (Santos, por exemplo) - Johnatas
            xcowsay --at=400,250 $"Em $CIDADE, $PAIS, a temperatura é de $TEMPERATURA_TRºC."
        fi
